    ...
    <div ref={containerRef} className="absolute inset-0 w-full pointer-events-none z-0 overflow-hidden select-none">
      ...
      <div 
        className={`absolute top-[-5%] right-[-5%] w-[680px] h-[680px] rounded-full blur-[140px] pointer-events-none transition-all duration-700 ${
          isLight ? 'bg-purple-200/60 opacity-90' : 'bg-fuchsia-900/25 opacity-60'
        }`} 
      />
      <div 
        className={`absolute top-[18%] left-[-8%] w-[750px] h-[750px] rounded-full blur-[150px] pointer-events-none transition-all duration-700 ${
          isLight ? 'bg-rose-200/70 opacity-95' : 'bg-rose-950/40 opacity-70'
        }`} 
      />
      <div 
        className={`absolute top-[48%] right-[5%] w-[650px] h-[650px] rounded-full blur-[150px] pointer-events-none transition-all duration-700 ${
          isLight ? 'bg-pink-200/65 opacity-90' : 'bg-pink-950/25 opacity-50'
        }`} 
      />
      <div 
        className={`absolute bottom-[10%] left-[12%] w-[700px] h-[700px] rounded-full blur-[160px] pointer-events-none transition-all duration-700 ${
          isLight ? 'bg-amber-100/60 opacity-80' : 'bg-purple-950/25 opacity-50'
        }`} 
      />
